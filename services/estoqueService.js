...
class EstoqueService {
    ...
    static async listarEstoqueGeral() {
        ...
        const mapaReservas = {};
        if (reservas) { reservas.forEach(r => { if(!mapaReservas[r.insumo_id]) mapaReservas[r.insumo_id] = 0; mapaReservas[r.insumo_id] += Number(r.quantidade_reservada); }); }
        ...
        insumos.forEach(ins => {
            ...
            if (!grupoMap[desc]) {
                grupoMap[desc] = { id_representante: ins.id, descricao: desc, unidade: ins.unidade, ref: ins.ref, ids: [], qtd_sede: 0, qtd_complexo: 0, qtd_separada: 0, tem_ligacao: false };
            }
            grupoMap[desc].ids.push(ins.id);
            if (idsComProduto.has(ins.id)) grupoMap[desc].tem_ligacao = true;
            const est = mapaEstoque[ins.id];
            if (est) { grupoMap[desc].qtd_sede += Number(est.qtd_sede || 0); grupoMap[desc].qtd_complexo += Number(est.qtd_complexo || 0); }
            const res = mapaReservas[ins.id];
            if (res) { grupoMap[desc].qtd_separada += Number(res); }
        });

        return Object.values(grupoMap).filter(item => item.tem_ligacao).map(item => ({ ...item, id: item.id_representante, total_geral: item.qtd_sede + item.qtd_complexo })).sort((a, b) => a.descricao.localeCompare(b.descricao));
    }
    ...
    static async mapearUsoPorProcesso() {
        ...
        procProdutos.forEach(pp => {
            ...
            comps.forEach(c => { if(!map[c.insumo_id]) map[c.insumo_id] = []; if(!map[c.insumo_id].includes(pp.processo_id)) map[c.insumo_id].push(pp.processo_id); });
        });
    ...
    static async registrarEntradaGeral(insumoId, local, quantidade, usuario) {
        ...
        if (atual) {
            ...
            const insertData = { insumo_id: insumoId, qtd_sede: local === 'SEDE' ? quantidade : 0, qtd_complexo: local === 'COMPLEXO' ? 0 : 0 };
            if (local === 'COMPLEXO') insertData.qtd_complexo = quantidade;
    ...
    static async listarReservasProcesso(processoId) {
        ...
        processo.insumos_separacao = processo.insumos_consolidados.map(ins => {
            ...
            const geral = mapaGeral[ins.insumo_id] || { qtd_sede: 0, qtd_complexo: 0 };
            const pendenteSeparacao = ins.qtd_arredondada - reservado;
            return { ...ins, quantidade_reservada: reservado, quantidade_pendente_separacao: pendenteSeparacao > 0 ? pendenteSeparacao : 0, saldo_sede: geral.qtd_sede, saldo_complexo: geral.qtd_complexo, status_separacao: reservado >= ins.qtd_arredondada };
        });
        return processo;
    }
    ...
    static async separarParaProcesso(processoId, insumoId, local, quantidade, usuario) {
        ...
        const saldoLocal = local === 'SEDE' ? geral.qtd_sede : geral.qtd_complexo;
    ...
    static async editarReserva(reservaId, novaQuantidade, localAjuste, usuario) {
        ...
        const updatesGeral = { updated_at: new Date().toISOString() };
        if (diff > 0) { 
            const saldo = localAjuste === 'SEDE' ? Number(geral.qtd_sede) : Number(geral.qtd_complexo);
    ...
    static async obterDetalhesInsumo(insumoId) {
        ...
        let qtd_sede = 0; let qtd_complexo = 0;
        if (estoqueList) { estoqueList.forEach(e => { qtd_sede += Number(e.qtd_sede || 0); qtd_complexo += Number(e.qtd_complexo || 0); }); }
        insumoBase.estoque = { qtd_sede, qtd_complexo };
        ...
        if (reservas && reservas.length > 0) {
            ...
            if (processosReserva) { separacoes = processosReserva.map(p => ({ reserva_id: sepMap[p.id].id, processo_id: p.id, numero: p.numero, nome: p.nome, status: p.status, quantidade_separada: sepMap[p.id].qtd })); }
        }
        ...
        if (!composicoes || composicoes.length === 0) return insumoBase;
    ...
    // ==========================================
    // SISTEMA DE ENVIOS EM LOTE E COMPROVANTES
    // ==========================================
    ...
    // NOVO: Realiza o desconto no banco para um ARRAY (lista) de insumos enviados juntos
    static async registrarEnvioProducaoLote(processoId, loteEnvios, entreguePor, recebidoPor) {
        const reciboId = Date.now().toString(); // Identificador único deste envio em lote
        const insercoesMov = [];
        ...
        for (let envio of loteEnvios) {
            ...
            // Salva na observação o LOTE para podermos buscar depois
            insercoesMov.push({
                tipo: 'SAIDA_PRODUCAO',
    ...
    // NOVO: Busca o histórico desviando do erro de cross-schema do banco e agrupando por lote
    static async obterHistoricoEnvios(processoId) {
        ...
        // Busca o nome dos insumos manualmente
        ...
        movs.forEach(mov => {
            ...
            const loteId = match ? match[1] : mov.id; // Se não tiver Lote (envios antigos), usa o próprio ID
    ...
    // NOVO: Prepara as informações para desenhar o PDF de um LOTE com vários materiais
    static async obterComprovanteEnvioLote(loteId) {
        let movs = [];
        
        // Tenta buscar pelo texto do Lote
        ...
        if (movsLote && movsLote.length > 0) {
            ...
            // Se for um envio antigo, busca pelo ID normal
